...
class Dominance:
    ...
    def __init__(
        self, data, target, top_k=None, objective=1, pseudo_r2="mcfadden", data_format=0
    ):  # Bala changes
        ...
        if self.data_format == 0:
            ...
            assert (self.top_k > 1) and (
                self.top_k < (len(self.data.columns))
            ), "Value of top_k ranges from 1 to n-1 !"
        self.complete_model_rsquare()

    def conditional_dominance(
        self, model_rsquares, model_features_k, model_features_k_minus_1, columns
    ):
        ...
        interactional_dominance = dict(
            {
                (
                    " ".join(set(model_features_k[0]) - set(i)),
    ...
    def individual_dominance(self, model_rsquares, model_features, columns):
        ...
        return dict(
            {(" ".join(col), model_rsquares[" ".join(col)]) for col in model_features}
        )

    def partial_dominance(
        self, model_rsquares, model_features_k, model_features_k_minus_1, columns
    ):
        # print(columns)
        pd = {col: [] for col in columns}
        [
            pd[" ".join(set(i) - set(j))].append(
                model_rsquares[" ".join(i)] - model_rsquares[" ".join(j)]
            )
            ...
            for j in model_features_k_minus_1:
                if len(set(i) - set(j)) == 1:
                    pd_comp_dom[" ".join(set(i) - set(j))].update(
                        {
                            " ".join(j): model_rsquares[" ".join(i)]
                            - model_rsquares[" ".join(j)]
                        }
                    )
        ...
        if self.objective == 1:
            for i in tqdm(model_combinations):
                for j in i:
                    ...
                    r_squared = lin_reg.score(
                        self.data[list(j)], self.data[self.target]
                    )
    ...
    def variable_statistics(self, model_rsquares, columns):
        ...
        for k in tqdm(range(len(columns), 1, -1)):
            ...
            model_features_k_minus_1 = [
                i for j in model_combinations for i in j if len(i) == k - 1
            ]
            if k == len(columns):
                (
                    stats["conditional_dominance"],
                    complete_dominance_stats["interactional_dominance"],
                ) = self.conditional_dominance(
                    model_rsquares, model_features_k, model_features_k_minus_1, columns
                )
            else:
                (
                    stats["partial_dominance_" + str(k)],
                    complete_dominance_stats["partial_dominance_" + str(k)],
                ) = self.partial_dominance(
                    model_rsquares, model_features_k, model_features_k_minus_1, columns
                )

            if k == 2:
                stats["individual_dominance"] = self.individual_dominance(
                    model_rsquares, model_features_k_minus_1, columns
                )
                complete_dominance_stats["individual_dominance"] = stats[
        ...
        for col in columns:
            variable_stats[col] = {
                ...
                "partial_dominance": [
                    np.mean(stats["partial_dominance_" + str(i)][col])
    ...
    def dominance_stats(self):
        ...
        tf["Average Partial Dominance"] = tf["partial_dominance"].apply(
            lambda x: np.mean(x)
        )
        tf["partial_dominance_count"] = tf["partial_dominance"].apply(lambda x: len(x))
        tf["Total Dominance"] = (
            tf["partial_dominance_count"] * tf["Average Partial Dominance"]
            + tf["conditional_dominance"]
            + tf["individual_dominance"]
        ...
        ].sort_values("Total Dominance", ascending=False)
        tf.rename(
            columns={"individual_dominance": "Individual Dominance"}, inplace=True
        )
        return tf
    ...
    def plot_waterfall_relative_importance(self, incremental_rsquare_df):
        ...
        data = {
            "Percentage Relative Importance": list(
                incremental_rsquare_df["percentage_incremental_r2"].values
            )
        }
        ...
        df.loc[df["Percentage Relative Importance"] < 0, "label_pos"] = (
            df.label_pos - 10000
        )
    ...
    def plot_incremental_rsquare(self):
        ...
        incremental_rsquare_df1.sort_values(
            "incremental_r2", ascending=False, inplace=True
        )

        incremental_rsquare_df2 = pd.DataFrame()
        incremental_rsquare_df2["Features"] = self.percentage_incremental_r2.keys()
        incremental_rsquare_df2[
            "percentage_incremental_r2"
        ] = self.percentage_incremental_r2.values()
        incremental_rsquare_df2.sort_values(
            "percentage_incremental_r2", ascending=False, inplace=True
        )
        ...
        incremental_rsquare_df["percentage_incremental_r2"] = (
            incremental_rsquare_df["percentage_incremental_r2"] * 100
        )
        # Bala changes start
        if self.data_format == 0:
            iplot(
                incremental_rsquare_df[["Features", "incremental_r2"]]
                .set_index("Features")
                .iplot(
                    asFigure=True,
                    kind="bar",
                    title="Incremetal "
                    + ("Pseudo " if (self.objective != 1) else " ")
                    + "R Squared for Top "
                    + str(self.top_k)
                    + " Variables ",
            ...
            iplot(
                incremental_rsquare_df[["Features", "percentage_incremental_r2"]].iplot(
                    asFigure=True,
                    kind="pie",
                    title="Percentage Relative Importance for Top "
                    + str(self.top_k)
                    + " Variables ",
        ...
        self.plot_waterfall_relative_importance(
            incremental_rsquare_df[["Features", "percentage_incremental_r2"]]
        )
        ...
        for x in general_dominance:
            ...
            if len(x["Dominating"]) > 0:
                for j in x["Dominating"]:
                    if (
                        self.variable_stats[predictor]["individual_dominance"]
                        < self.variable_stats[j]["individual_dominance"]
                    ) or (
                        self.variable_stats[predictor]["conditional_dominance"]
                        < self.variable_stats[j]["conditional_dominance"]
                    ):
                        cd = False
                        break
    ...
    def predict_complete_dominance(self):
        ...
        conditional_dominant_predictors = list(
            conditional_dominance_df[
                conditional_dominance_df["Conditional Dominance"] == True
            ]["Predictors"].values
        )
        ...
        for i in conditional_dominant_predictors:
            ...
            dominating = conditional_dominance_df[
                conditional_dominance_df["Predictors"] == i
            ]["Dominating"].values[0]
            complete_dominance = True
            for j in [p for p in list(cds.keys()) if p != "interactional_dominance"]:
                if j == "individual_dominance":
                    if sum(cds[j][i] > [cds[j][key] for key in dominating]) != len(
                        dominating
                    ):
                    ...
                    for k in dominating:
                        if complete_dominance:
                            for key in cds[j][i].keys():
                                l = list(
                                    set(predictors) - set(key.split(" ")) - set([i])
                                )
                    ...
                    if complete_dominance:
                        for search in search_index:
                            ...
                            if (
                                cds[j][search[0]][search[1]]
                                < cds[j][search[2]][search[1]]
                            ):
                                complete_dominance = False
                                break
    ...
    def dominance_level(self):
        ...
        condition_dom.rename(
            columns={"Dominating": "Conditionally Dominating"}, inplace=True
        )
        ...
        return pd.merge(
            pd.merge(
                left=gen_dom,
                right=condition_dom[["Predictors", "Conditionally Dominating"]],
                how="left",
            ),
            comp_dom,
            how="left",
    ...
    def incremental_rsquare(self):
        ...
        if self.data_format == 0:  # Bala changes
            ...
            print()
            print(
                "Creating models for %s possible combinations of %s features :"
        ...
        for col in columns:
            l = [
                variable_stats[col]["individual_dominance"],
        ...
        self.percentage_incremental_r2 = {
            col: incrimental_r2[col] / np.sum(list(incrimental_r2.values()))
            for col in columns
        }
    ...
    def complete_model_rsquare(self):
        if self.data_format == 0:  # Bala changes
            ...
            if self.objective == 1:
                ...
                if self.pseudo_r2 == "mcfadden":
                    print(
                        "MacFadden's R-Squared : %s " % (self.McFadden_RSquare(columns))
                    )
                elif pseudo_r2 == "nagelkerke":
                    print(
                        "Nagelkerke R-Squared : %s "
                        % (self.Nagelkerke_Rsquare(columns))
                    )
                elif pseudo_r2 == "cox_and_snell":
                    print(
                        "Cox and Snell R-Squared : %s "
                        % (self.Cox_and_Snell_Rsquare(columns))
                    )
                else:
        ...
        # Bala changes start
            ...
            corr_pred_matrix_inverse = pd.DataFrame(
                np.linalg.pinv(corr_pred_matrix.values),
    ...
    def Dominance_correlation(self):
        ...
        print(
            "Calculating R2 for %s possible combinations of %s features :"
            % ((2 ** len(columns)) - 1, len(columns))
        )
        model_combinations = self.model_features_combination(columns)
        model_rsquares = {}
...
class Dominance_Datasets:
    ...
    def get_breast_cancer(cls):
        ...
        breast_cancer_data["target_names"] = breast_cancer_data["target"].map(
            target_dict
        )
    ...
    def get_boston(cls):
        ...
        boston_data = pd.DataFrame(
            data=load_boston()["data"], columns=load_boston()["feature_names"]
        )
